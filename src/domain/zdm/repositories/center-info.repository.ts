...
export class ZdmRepository extends CommonRepository {
  protected readonly tableName = "center_info"
  ...
  async findAll({ filterOptions }: { filterOptions: ZdmFilterOptions }): Promise<ZdmInfoTable[]> {
    try {
      ...
      return await executeQuery<ZdmInfoTable>({ sql: query, params: this.params })
    } catch (error) {
      ContextLogger.debug({
  ...
  async findByZdmName({ name, filterOptions }: { name: string; filterOptions: ZdmFilterOptions }): Promise<ZdmInfoTable | null> {
    try {
      ...
      let query = `SELECT * FROM ${this.tableName}`
      query += this.buildWhereClause()
      return await executeQuerySingle<ZdmInfoTable>({ sql: query, params: this.params })
    } catch (error) {
      ContextLogger.debug({
